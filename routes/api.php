...
Route::middleware(['auth:sanctum', 'api.limit:standard', 'log.activity'])->group(function () {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // PURCHASE ORDERS
    // Urutan penting: route statis (/summary) harus sebelum route parameter
    // (/{purchaseOrder}) agar tidak tertangkap sebagai ID.
    // ═══════════════════════════════════════════════════════════════════════
    Route::prefix('purchase-orders')->group(function () {
        Route::get('/',                        [PurchaseOrderController::class, 'index']);
        Route::post('/',                       [PurchaseOrderController::class, 'store']);

        // ── Route statis — harus di atas route parameter ──────────────────
        Route::get('/summary',                 [PurchaseOrderController::class, 'summary']);       // ← BARU
    ...
    Route::prefix('surat-jalan')->group(function () {
        ...
        Route::get('/po/{po}/remaining',       [SuratJalanController::class, 'remaining']); // statis sebelum parameter
    ...
    Route::post('/suppliers', function (Request $req) {
        if (!$req->user()->hasPermissionTo('manage-accounting'))
            return response()->json(['success' => false, 'message' => 'Tidak memiliki akses'], 403);
        $v = $req->validate(['code' => 'required|string|max:20|unique:suppliers', 'name' => 'required|string|max:255', 'contact_name' => 'nullable|string|max:100', 'phone' => 'nullable|string|max:20', 'email' => 'nullable|email|max:100', 'address' => 'nullable|string', 'npwp' => 'nullable|string|max:30', 'notes' => 'nullable|string']);
    ...
    Route::put('/suppliers/{supplier}', function (Request $req, Supplier $supplier) {
        if (!$req->user()->hasPermissionTo('manage-accounting'))
            return response()->json(['success' => false, 'message' => 'Tidak memiliki akses'], 403);
        $supplier->update($req->validate(['name' => 'sometimes|string|max:255', 'contact_name' => 'nullable|string|max:100', 'phone' => 'nullable|string|max:20', 'email' => 'nullable|email|max:100', 'address' => 'nullable|string', 'npwp' => 'nullable|string|max:30', 'is_active' => 'sometimes|boolean', 'notes' => 'nullable|string']));
    ...
    Route::post('/supplier-invoices', function (Request $req) {
        ...
        if (!empty($v['purchase_order_id'])) {
            ...
            if ($po->status === 'completed')
                return response()->json(['success' => false, 'message' => "PO {$po->po_number} sudah selesai/lunas"], 422);
            if ($po->status === 'cancelled')
                return response()->json(['success' => false, 'message' => "PO {$po->po_number} sudah dibatalkan"], 422);
            $totalInvoiced = SupplierInvoice::where('purchase_order_id', $po->id)->whereNotIn('status', ['cancelled'])->sum('total_amount');
            $sisaBisaInvoice = (float) $po->grand_total - $totalInvoiced;
            if ($sisaBisaInvoice <= 0)
                return response()->json(['success' => false, 'message' => "PO {$po->po_number} sudah sepenuhnya diinvoice"], 422);
            if ($v['total_amount'] > $sisaBisaInvoice)
                return response()->json(['success' => false, 'message' => 'Nilai invoice melebihi sisa nilai PO. Sisa: Rp ' . number_format($sisaBisaInvoice, 0, ',', '.')], 422);
        }
    ...
    Route::post('/supplier-payments', function (Request $req) {
        if (!$req->user()->hasPermissionTo('manage-accounting'))
            return response()->json(['success' => false, 'message' => 'Tidak memiliki akses'], 403);
        $v = $req->validate(['supplier_id' => 'required|exists:suppliers,id', 'supplier_invoice_id' => 'required|exists:supplier_invoices,id', 'main_cash_account_id' => 'nullable|exists:main_cash_accounts,id', 'amount' => 'required|numeric|min:1', 'payment_date' => 'required|date', 'payment_method' => 'required|in:transfer,cash,giro,cek', 'reference_number' => 'nullable|string', 'notes' => 'nullable|string']);
        $invoice = SupplierInvoice::findOrFail($v['supplier_invoice_id']);
        if ($invoice->status === 'paid')      return response()->json(['success' => false, 'message' => 'Invoice sudah lunas'], 422);
        if ($invoice->status === 'cancelled') return response()->json(['success' => false, 'message' => 'Invoice sudah dibatalkan'], 422);
        if ($invoice->remaining_amount <= 0)  return response()->json(['success' => false, 'message' => 'Sisa tagihan sudah nol'], 422);
    ...
    Route::post('/supplier-payments/{payment}/approve', function (Request $req, SupplierPayment $payment) {
        if (!$req->user()->hasPermissionTo('approve-accounting'))
            return response()->json(['success' => false, 'message' => 'Tidak memiliki akses'], 403);
        if ($payment->status !== 'pending')           return response()->json(['success' => false, 'message' => 'Pembayaran sudah diproses'], 422);
        if ($payment->invoice->status === 'paid')     return response()->json(['success' => false, 'message' => 'Invoice terkait sudah lunas'], 422);
        if ($payment->invoice->remaining_amount <= 0) return response()->json(['success' => false, 'message' => 'Sisa tagihan sudah nol'], 422);
    ...
    Route::post('/supplier-payments/{payment}/reject', function (Request $req, SupplierPayment $payment) {
        ...
        $payment->update(['status' => 'rejected', 'approved_by' => $req->user()->id, 'approved_at' => now(), 'notes' => $req->notes ? "Ditolak: {$req->notes}" : 'Ditolak oleh accounting']);
    ...
    Route::post('/petty-cash/transactions', function (Request $req) {
        if (!$req->user()->hasPermissionTo('manage-accounting'))
            return response()->json(['success' => false, 'message' => 'Tidak memiliki akses'], 403);
        $v = $req->validate(['petty_cash_account_id' => 'required|exists:petty_cash_accounts,id', 'main_cash_account_id' => 'nullable|exists:main_cash_accounts,id', 'type' => 'required|in:in,out', 'amount' => 'required|numeric|min:1', 'description' => 'required|string', 'reference_number' => 'nullable|string', 'transaction_date' => 'required|date', 'notes' => 'nullable|string']);
        ...
        if ($v['type'] === 'in' && !empty($v['main_cash_account_id'])) {
            ...
            if ($mainAcc->balance < $v['amount'])
                return response()->json(['success' => false, 'message' => "Saldo rekening {$mainAcc->name} tidak cukup (saldo: Rp " . number_format($mainAcc->balance, 0, ',', '.') . ")"], 422);
        }
    ...
    Route::post('/main-cash/transactions', function (Request $req) {
        if (!$req->user()->hasPermissionTo('manage-accounting'))
            return response()->json(['success' => false, 'message' => 'Tidak memiliki akses'], 403);
        $v = $req->validate(['main_cash_account_id' => 'required|exists:main_cash_accounts,id', 'type' => 'required|in:in,out', 'amount' => 'required|numeric|min:1', 'description' => 'required|string', 'reference_number' => 'nullable|string', 'transaction_date' => 'required|date', 'notes' => 'nullable|string']);
    ...
    Route::post('/journal-entries', function (Request $req) {
        if (!$req->user()->hasPermissionTo('manage-accounting'))
            return response()->json(['success' => false, 'message' => 'Tidak memiliki akses'], 403);
        $v = $req->validate(['entry_date' => 'required|date', 'description' => 'required|string|max:255', 'notes' => 'nullable|string', 'items' => 'required|array|min:2', 'items.*.account_code' => 'required|string|max:20', 'items.*.account_name' => 'required|string|max:100', 'items.*.account_type' => 'required|in:asset,liability,equity,revenue,expense', 'items.*.debit' => 'required|numeric|min:0', 'items.*.credit' => 'required|numeric|min:0', 'items.*.description' => 'nullable|string']);
        ...
        $entry = \App\Models\JournalEntry::create(['journal_number' => \App\Models\JournalEntry::generateNumber(), 'entry_date' => $v['entry_date'], 'description' => $v['description'], 'total_debit' => $totalDebit, 'total_credit' => $totalCredit, 'status' => 'draft', 'created_by' => $req->user()->id, 'notes' => $v['notes'] ?? null]);
    ...
    Route::get('/general-ledger', function (Request $req) {
        ...
        $q = \App\Models\JournalItem::query()
            ...
            ->select('journal_items.account_code', 'journal_items.account_name', 'journal_items.account_type', \Illuminate\Support\Facades\DB::raw('SUM(journal_items.debit) as total_debit'), \Illuminate\Support\Facades\DB::raw('SUM(journal_items.credit) as total_credit'), \Illuminate\Support\Facades\DB::raw('SUM(journal_items.debit) - SUM(journal_items.credit) as balance'))
    ...
    Route::get('/reports/cash-flow', function (Request $req) {
        ...
        $mainIn   = MainCashTransaction::where('status', 'approved')->where('type', 'in')->whereBetween('transaction_date', [$dateFrom, $dateTo])->sum('amount');
        $mainOut  = MainCashTransaction::where('status', 'approved')->where('type', 'out')->whereBetween('transaction_date', [$dateFrom, $dateTo])->sum('amount');
        $pettyIn  = PettyCashTransaction::where('status', 'approved')->where('type', 'in')->whereBetween('transaction_date', [$dateFrom, $dateTo])->sum('amount');
        $pettyOut = PettyCashTransaction::where('status', 'approved')->where('type', 'out')->whereBetween('transaction_date', [$dateFrom, $dateTo])->sum('amount');
        $supplierPaid = SupplierPayment::where('status', 'approved')->whereBetween('payment_date', [$dateFrom, $dateTo])->sum('amount');
        $payrollCost  = PayrollItem::whereHas('period', fn($q) => $q->where('status', 'paid')->whereBetween('payment_date', [$dateFrom, $dateTo]))->sum('net_salary');
        $transactions = MainCashTransaction::with('account', 'creator')->where('status', 'approved')->whereBetween('transaction_date', [$dateFrom, $dateTo])->orderBy('transaction_date', 'desc')->get();
        return response()->json(['success' => true, 'data' => ['period' => ['from' => $dateFrom, 'to' => $dateTo], 'main_cash_in' => $mainIn, 'main_cash_out' => $mainOut, 'petty_cash_in' => $pettyIn, 'petty_cash_out' => $pettyOut, 'supplier_paid' => $supplierPaid, 'payroll_cost' => $payrollCost, 'net_cash_flow' => ($mainIn + $pettyIn) - ($mainOut + $pettyOut), 'transactions' => $transactions]]);
    });
    ...
    Route::get('/reports/payroll-summary', function (Request $req) {
        ...
        $periods = PayrollPeriod::with('items.employee')->where('year', $year)->orderBy('month')->get()
            ->map(fn($p) => ['id' => $p->id, 'name' => $p->name, 'month' => $p->month, 'year' => $p->year, 'status' => $p->status, 'payment_date' => $p->payment_date, 'employee_count' => $p->items->count(), 'total_gross' => $p->items->sum('gross_salary'), 'total_deduction' => $p->items->sum('total_deduction'), 'total_net' => $p->items->sum('net_salary'), 'total_bpjs' => $p->items->sum(fn($i) => $i->deduction_bpjs_tk + $i->deduction_bpjs_kes), 'total_loan' => $p->items->sum('deduction_loan')]);
        return response()->json(['success' => true, 'data' => ['year' => $year, 'periods' => $periods, 'annual' => ['total_gross' => $periods->sum('total_gross'), 'total_net' => $periods->sum('total_net')]]]);
    });
    ...
    Route::get('/reports/expense', function (Request $req) {
        ...
        $dateFrom      = $req->date_from ?? now()->startOfMonth()->toDateString();
        $dateTo        = $req->date_to   ?? now()->endOfMonth()->toDateString();
        $mainExpenses  = MainCashTransaction::where('status', 'approved')->where('type', 'out')->whereBetween('transaction_date', [$dateFrom, $dateTo])->with('account', 'creator')->orderBy('transaction_date', 'desc')->get();
        $pettyExpenses = PettyCashTransaction::where('status', 'approved')->where('type', 'out')->whereBetween('transaction_date', [$dateFrom, $dateTo])->with('account', 'creator')->orderBy('transaction_date', 'desc')->get();
        return response()->json(['success' => true, 'data' => ['period' => ['from' => $dateFrom, 'to' => $dateTo], 'main_expenses' => $mainExpenses, 'petty_expenses' => $pettyExpenses, 'total_main' => $mainExpenses->sum('amount'), 'total_petty' => $pettyExpenses->sum('amount'), 'grand_total' => $mainExpenses->sum('amount') + $pettyExpenses->sum('amount')]]);
    });

    // PO for Invoice
    Route::get('/purchase-orders/{purchaseOrder}/for-invoice', function (\App\Models\PurchaseOrder $purchaseOrder) {
        $invoicedAmount       = SupplierInvoice::where('purchase_order_id', $purchaseOrder->id)->whereNotIn('status', ['cancelled'])->sum('total_amount');
        $remainingInvoiceable = max(0, (float) $purchaseOrder->grand_total - $invoicedAmount);
        return response()->json(['success' => true, 'data' => ['id' => $purchaseOrder->id, 'po_number' => $purchaseOrder->po_number, 'vendor_name' => $purchaseOrder->vendor_name, 'grand_total' => $purchaseOrder->grand_total, 'ppn_amount' => $purchaseOrder->ppn_amount, 'subtotal' => $purchaseOrder->total_amount, 'status' => $purchaseOrder->status, 'invoiced_amount' => (float) $invoicedAmount, 'remaining_invoiceable' => $remainingInvoiceable, 'fully_invoiced' => $invoicedAmount >= (float) $purchaseOrder->grand_total && (float) $purchaseOrder->grand_total > 0]]);
    });
    ...
    Route::get('/purchase-orders-by-supplier/{supplier}', function (\App\Models\Supplier $supplier) {
        ...
        $pos = \App\Models\PurchaseOrder::whereNotIn('status', ['cancelled'])
            ->where('vendor_name', 'ilike', "%{$supplierName}%")
            ->orderBy('created_at', 'desc')
        ...
        $filtered = $pos->filter(function ($po) use ($supplierName) {
            ...
            return str_contains($vendor, $supplier) || (strlen($vendor) >= 5 && str_contains($supplier, $vendor));
        })->values()->map(function ($po) {
            $invoicedAmount        = SupplierInvoice::where('purchase_order_id', $po->id)->whereNotIn('status', ['cancelled'])->sum('total_amount');
    ...
    // Utility: Recalculate avg_price
    Route::post('/utility/recalculate-avg-price', function (Request $request) {
        $dryRun  = $request->boolean('dry_run', false);
        $results = [];
        \Illuminate\Support\Facades\DB::transaction(function () use ($dryRun, &$results) {
            $groups = \App\Models\ItemPriceHistory::select('item_id', 'warehouse_id')->groupBy('item_id', 'warehouse_id')->get();
            $itemAvgMap = [];
            foreach ($groups as $g) {
                $prices    = \App\Models\ItemPriceHistory::where('item_id', $g->item_id)->where('warehouse_id', $g->warehouse_id)->pluck('purchase_price')->map(fn($p) => (float) $p);
                $simpleAvg = $prices->avg();
                $stock     = \App\Models\ItemStock::where('item_id', $g->item_id)->where('warehouse_id', $g->warehouse_id)->first();
                if ($stock) {
                    $results[] = ['item_id' => $g->item_id, 'warehouse_id' => $g->warehouse_id, 'avg_lama' => (float) $stock->avg_price, 'avg_baru' => $simpleAvg, 'n_transaksi' => $prices->count()];
                    if (!$dryRun) $stock->update(['avg_price' => $simpleAvg]);
                }
            ...
            foreach ($itemAvgMap as $itemId => $avgList) {
                ...
                if ($item) {
                    foreach ($results as &$r) {
                        if ($r['item_id'] === $itemId) { $r['item_name'] = $item->name; $r['price_lama'] = (float) $item->price; $r['price_baru'] = $itemAvg; }
                    }
                    unset($r);
                    if (!$dryRun) $item->update(['price' => $itemAvg]);
                }
            }
        });
