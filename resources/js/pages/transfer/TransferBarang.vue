...
async function printMRDirect(mr) {
  try {
    const res = await axios.get('/transfer-barang/' + mr.id)
    pendingPrintMR = res.data.data
    openSignerPicker('transfer_barang', pendingPrintMR?.id)
  } catch { toast.error('Gagal memuat data') }
}

function doPrintMR(slots = []) {
  // Konversi slots ke resolvedSigners (filter null)
  const resolvedSigners = slots.filter(s => s !== null)
  const mr = pendingPrintMR
  if (!mr) return
  pendingPrintMR = null

  printMR(mr, resolvedSigners)
}

function buildSignBoxesTF(resolvedSigners, defaultLabels) {
...
function printMR(mr, resolvedSigners = []) {
  ...
  const html =
    ...
    '<table class="it">'+
      ...
      '<tbody>'+rows+'</tbody>'+
    '</table>'+
        buildSignBoxesTF(resolvedSigners, ['Dikirim Oleh','Driver','Diterima Oleh','Mengetahui'])+
    bdC+htC
