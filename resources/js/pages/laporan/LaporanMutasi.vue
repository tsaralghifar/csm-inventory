        ...
        <div class="row g-2 align-items-end">
          ...
          <div class="col-md-2">
            <select v-model="params.type" class="form-select form-select-sm">
              ...
              <option value="out_apd">Keluar - APD</option>
          ...
          <table class="table csm-table mb-0">
            ...
            <tbody>
              ...
              <tr v-for="m in data" :key="m.id">
                ...
                <td>
                  <!-- Badge jenis mutasi — APD diberi warna khusus ungu agar mudah dibedakan -->
                  <span :class="typeClass(m.type, m.moveable_type)" class="badge small">
                    {{ typeLabel(m.type, m.moveable_type) }}
                  </span>
                </td>
                <td>
                  <!-- Keterangan/notes — penting untuk APD agar karyawan penerima terlihat -->
                ...
                <td class="text-end">
                  <span :class="parseFloat(m.qty_after) < 0 ? 'stock-minus' : 'fw-semibold'">
                    {{ $formatNumber(m.qty_after) }}
                  </span>
                </td>
...
async function load() {
  loading.value = true
  try {
    // Filter 'out_apd' dikirim ke backend sebagai type=out + moveable_type=apd_distribution
    const apiParams = { ...params.value, page: meta.value.page, per_page: 50 }
    if (params.value.type === 'out_apd') {
      apiParams.type          = 'out'
      apiParams.moveable_type = 'apd_distribution'
    }

    const r = await axios.get('/reports/movements', { params: apiParams })
...
/**
 * Label jenis mutasi.
 * APD ditampilkan sebagai "Keluar - APD" agar bisa dibedakan dari pengeluaran bon biasa.
 */
function typeLabel(type, moveableType) {
  if (type === 'out' && moveableType === 'apd_distribution') return 'Keluar - APD'
  const map = {
    in:           'Masuk',
    out:          'Keluar',
...
/**
 * Warna badge per jenis mutasi.
 * APD: ungu (bg-purple) agar mudah dibedakan secara visual.
 */
function typeClass(type, moveableType) {
  if (type === 'out' && moveableType === 'apd_distribution') return 'text-white' + ' ' + 'bg-purple'
  if (type === 'in'  || type === 'transfer_in')  return 'bg-success'
  if (type === 'out' || type === 'transfer_out') return 'bg-warning text-dark'
  return 'bg-secondary'
}

function exportExcel() {
  const headers = [
    'Tanggal', 'Ref No', 'Jenis', 'Keterangan',
    'Part Number', 'Nama Barang',
    'Dari Gudang', 'Ke Gudang',
    'Qty', 'Harga FIFO', 'Metode', 'Stok Sesudah', 'Oleh',
  ]
